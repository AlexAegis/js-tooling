...
export const setupVite = async (rawOptions?: DistributeInWorkspaceOptions): Promise<void> => {
	...
	await Promise.all([
		...
		distributePackageJsonItemsInWorkspace(
			{
				scripts: {
					build: 'turbo run build-lib_ --concurrency 6 --cache-dir .cache/turbo --filter ${packageName}',
					build_: undefined,
					'build-lib': undefined,
		...
		distributePackageJsonItemsInWorkspace(
			{
				scripts: {
					build: 'turbo run build-app_ --concurrency 6 --cache-dir .cache/turbo --filter ${packageName}',
					build_: undefined,
					'build-app': undefined,
					'build-app_': 'vite build',
					dev: 'concurrently npm:watch-deps npm:start',
					'watch-deps':
						"nodemon --config ${relativePathFromPackageToRoot}/node_modules/@alexaegis/setup-vite/static/nodemon.json --watch ./node_modules/**/src/**/* --ext ts,tsx,mts,cts,svelte,js,jsx,mjs,cjs,json --ignore dist --exec 'turbo run build-lib_ --concurrency 6 --cache-dir .cache/turbo --filter ${packageName}'",
					start: 'vite',
				},
				devDependencies: {
					['@alexaegis/vite']: packageJson.devDependencies['@alexaegis/vite'],
